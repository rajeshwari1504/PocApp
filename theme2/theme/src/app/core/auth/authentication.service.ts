...
import { Data, Salary } from '../../content/pages/components/dashboard/salary.model';
...
export class AuthenticationService implements AuthService {
	...
	url= "http://localhost:3000/update-emp"
	...
	// }
	 ...
	 public login(login:Login): Observable<any>{    
		 console.log(login)
		return this.http.post<loginData>("http://localhost:3000/emp-login" , login)
		.pipe(
					map((result: any) => {
						console.log(result)
						if (result instanceof Array) {
	...
	//   **********update fuction  end for employees**********
	salary= new Subject<Salary>();
   salaryList(id:any) :Observable<Data[]>{
	   return this.http.post<Salary[]>("http://localhost:3000/emp-salary-dashboard",{id})
	   .pipe(
		map(data => data.map(o => this.toContact(o) ))
	  );
	}
  
	private toContact(obj: any): Data {
	  console.log(obj);
	  return {
		id: obj.id,
		empid: obj.empid,
		salarydate: obj.salarydate,
		amount: obj.amount,
   ...
   getDesignation()
   {
	 return this.http.get("http://localhost:3000/get-designation");
   }
